 ...
 * @author Josep
 */
public class IncluirHistoriaUsuarioCommand implements ICommandManterOS {

    private static IncluirHistoriaUsuarioCommand instance;

    public static IncluirHistoriaUsuarioCommand getInstance() {
        if (instance == null) {
            instance = new IncluirHistoriaUsuarioCommand();
        }
        return instance;
    }
    ...
    private void salvarInformacoesOS(ManterOrdemServicoPresenter presenter, OrdemServico os, Object ob1) {
        try {
            ...
            HistoriaUsuario historia = new HistoriaUsuario(presenter.getView().getJTextField1().getText(),
                    presenter.getView().getJTextField5().getText());

            DisciplinaHistoriaUsuario disciplina = new DisciplinaHistoriaUsuario(presenter.getView().getJTextField2().getText(),
